...
-- атрибуты "интервал" созданы исключительно дл€ внесени€ данных по триггеру 
...
last_name varchar(30),
first_name varchar(30),
home_address varchar(100), --домашний адрес работника, с указанием города (денормализаци€)
store_id int,
store_address varchar(100), --адрес филиала, где работает работник, с указанием города (денормализаци€)
time_come_in timestamp[], -- все входы работника за день (массив, денормализаци€)
time_come_out timestamp[], -- все выходы работника за день (массив, денормализаци€)
job_interval interval, -- рабочие часы за день
pay_on_hour numeric, -- оплата сотруднику в час
hours_at_month int, -- количество отработанных часов в мес€ц
salary numeric,
...
language plpgsql
 ...
 -- ѕосколько делать запрос автоматизированным не требуетс€, полагаю, такого представлени€
 -- должно хватить, чтоб получить актуальную информацию.  лючевой момент - врем€ входа
 -- должно быть not null, а врем€ выхода - null. 